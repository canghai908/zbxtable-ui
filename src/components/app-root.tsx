      ...
      <SidebarInset>
        <div className="flex min-h-svh flex-col bg-muted/30">
          <header className="sticky top-0 z-10">
            <div className="flex h-(--header-height) items-center gap-3 border-b bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/70 lg:px-6">
              <SidebarTrigger />
              <Separator orientation="vertical" className="h-6" />
              <Breadcrumb>
                <BreadcrumbList>
                  <BreadcrumbItem>
                    <BreadcrumbPage>{activeRoot?.name ?? systemName}</BreadcrumbPage>
                  </BreadcrumbItem>
                  {currentMenu && activeRoot && currentMenu.fullPath !== activeRoot.fullPath ? (
                    <BreadcrumbItem>
                      <BreadcrumbPage>{currentMenu.name ?? currentMenu.router}</BreadcrumbPage>
                    </BreadcrumbItem>
                  ) : null}
                </BreadcrumbList>
              </Breadcrumb>
              <div className="ml-auto hidden w-full max-w-sm items-center lg:flex">
                <div className="relative w-full">
                  ...
                  <Input className="pl-9" placeholder="搜索菜单、主机或页面…" />
                </div>
              </div>
